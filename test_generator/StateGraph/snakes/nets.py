...
class Place (Node) :
    ...
    def __init__ (self, name, constraint: Set(str) = set(), tokens=[], check=None) :
        ...
        @type check: `Type`
        """
        self.hashkey = None # Added this to keep the hash key while changing object attribute values
        self.name = name
    ...
    def __eq__(self, __o):
        if not isinstance(__o, Place):
            ...
            return self.name == __o.name and self.constraint == __o.constraint
    
    # Added by me
    def __hash__(self) -> int:
        if self.hashkey is None:
            self.hashkey = hash((self.name, self.tokens, self._check, tuple(self.constraint)))
            #return hash((self.name, self.tokens, self._check, tuple(self.constraint)))
        return self.hashkey
...
class Transition (Node) :
    ...
    def __eq__(self, __o: object) -> bool:
        if isinstance(__o, Transition):
            return self.name == __o.name and self.guard == __o.guard
        else:
            return False
...
class PetriNet (object) :
    ...
    def __init__ (self, name) :
        ...
        # Added by me
        self.merge_place_cnt = 0
        self.merge_action_cnt = 0
    ...
    def add_place (self, place) :
        ...
        @raise ConstraintError: when a place with the same name exists
            already in the net
        """
        def contain(new_place):
            new_assert = set(new_place.name.split(';'))
            for place_name in self._place:
                assertions = set(place_name.split(';'))
                if new_assert.issubset(assertions):
                    self._place[new_place.name] = self._place[place_name]
                    self._node[new_place.name] = self._node[place_name]
                    return True
                elif new_assert.issuperset(assertions):
                    self._place[place_name] = new_place
                    self._node[place_name] = new_place
                    return True
            return False

        # if place.name in self._place or contain(place):
        if place.name in self._place:
            # Added by me
            # raise ConstraintError("place '%s' exists" % place.name)
            tmpplace = self._place[place.name]
            if place.name != '' and place.name != 'initial-place' and len(tmpplace.post.keys()) >= 2 and len(tmpplace.pre.keys()) >= 2:
                self.merge_place_cnt += 1
            return
